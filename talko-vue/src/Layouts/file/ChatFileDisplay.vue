<template>
  <div class="chat-file-display">

    <div v-if="isImage" class="image-preview">
      <img :src="file.file_path" :alt="file.file_name || 'Image'" class="max-w-xs rounded" />
    </div>


    <div v-else-if="isVideo" class="video-preview">
      <video :src="file.file_path" controls class="max-w-xs rounded"></video>
    </div>

    <!-- PDF -->
    <a v-else-if="isPDF" :href="file.file_path" target="_blank" class="text-blue-500 underline flex items-center">
    ...
    <!--
    <a v-else-if="isWord" :href="file.file_path" target="_blank" class="text-blue-500 underline flex items-center">
      📄 {{ file.file_name || 'Word Document' }}
    </a>
    -->


    <a v-else :href="file.file_path" target="_blank" class="text-blue-500 underline flex items-center">
      📁 {{ file.file_name || 'File' }}
    </a>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    isVideo() {
      return this.file.file_type === 'video';
    },
    isPDF() {
      return this.file.file_type === 'pdf';
    },
    isWord() {
      return this.file.file_type === 'doc' || this.file.file_type === 'docx';
    },
  },
};
</script>
